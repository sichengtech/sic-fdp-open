...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxa}/oa/oaNotify${oaNotify.self?'/self.do':'.do'}">通知列表</a></li>
		<c:if test="${!oaNotify.self}"><shiro:hasPermission name="oa:oaNotify:edit"><li><a href="${ctxa}/oa/oaNotify/form.do">通知添加</a></li></shiro:hasPermission></c:if>
	</ul>
	<form:form id="searchForm" modelAttribute="oaNotify" action="${ctxa}/oa/oaNotify${oaNotify.self?'/self.do':'.do'}" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="oaNotify">
			<tr>
				<td><a href="${ctxa}/oa/oaNotify/${requestScope.oaNotify.self?'view':'form'}.do?id=${oaNotify.id}">
				...
				<c:if test="${!requestScope.oaNotify.self}"><shiro:hasPermission name="oa:oaNotify:edit"><td>
    				<a href="${ctxa}/oa/oaNotify/form.do?id=${oaNotify.id}">修改</a>
					<a href="javascript:;" onclick="fdp.confirm('确认要删除该通知吗？', '${ctxa}/oa/oaNotify/delete.do?id=${oaNotify.id}')">删除</a>
