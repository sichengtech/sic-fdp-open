...
<body>
	...
	<section class="panel">
		<header class="panel-heading custom-tab tab-right ">
			<h4 class="title">短信日志列表</h4>
			<%@ include file="../include/functionBtn.jsp"%>
			<ul class="nav nav-tabs pull-right">
				<li class="active"><a href="javaScript:;"> <i class="fa fa-user"></i> 短信日志列表</a></li>
				<shiro:hasPermission name="sys:sysSmsLog:edit"><li class=""><a href="${ctxa}/sys/sysSmsLog/save1.do" > <i class="fa fa-user"></i> 短信日志添加</a></li></shiro:hasPermission>
			</ul>
		</header>
				...
				<form action="${ctxa}/sys/sysSmsLog/list.do" method="get" id="searchForm">
					...
					<div class="col-sm-2">
						...
						<input name="beginSendDate" type="text" readonly="readonly" maxlength="20" class="form-control input-sm J-date-start"
							...
							onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:true});" placeholder="请选择开始时间"/>
					...
					<div class="col-sm-2">
						...
						<input name="endSendDate" type="text" readonly="readonly" maxlength="20" class="form-control input-sm J-date-start"
							...
							onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:true});" placeholder="请选择结束时间"/>
							<div class="input-group-addon"><i class="fa fa-calendar"></i></div>
						</div>
					</div>
			...
			<table class="table table-hover table-condensed table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td>${not empty sysSmsLog.templatecode?sysSmsLog.templatecode:'无'}</td>
						<td>${fns:getDictLabel(sysSmsLog.status, 'result_status', '')}</td>
						<td>${not empty sysSmsLog.bewrite?sysSmsLog.bewrite:'无'}</td>
