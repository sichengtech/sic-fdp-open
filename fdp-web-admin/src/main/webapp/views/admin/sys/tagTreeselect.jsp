...
<head>
	...
	<script type="text/javascript">
		var key, lastValue = "", nodeList = [], type = fdp.getQueryString("type", "${url}");
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				async:{enable:(type==3),url:"${ctxa}/sys/user/treeData.do",autoParam:["id=officeId"]},
		...
		$(document).ready(function(){
			$.getJSON("${ctxa}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&t="
...
<div class="form-group">
	<div style="position:absolute;right:8px;top:5px;cursor:pointer;display: none" id="searchBtn" onclick="search();">
		<i class="icon-search"></i><label id="txt">搜索</label>
	</div>
	<div id="search" class="form-search" style="padding:10px 0 0 13px;">
		<label for="key" class="control-label" style="padding:5px 10px 3px 0;">关键字：</label>
		<input type="text" class="form-control input-sm" id="key" name="key" maxlength="50" style="width:110px;">
		<button class="btn btn-info" id="btn" onclick="searchNode()" style="position:absolute;right:17px;top:8px;cursor:pointer;">
			<i class="fa fa-search"></i>搜索
		</button>
	</div>
	<div id="tree" class="ztree" style="padding:15px 20px;overflow-y: auto;height: 314px;"></div>
</div>
</body>