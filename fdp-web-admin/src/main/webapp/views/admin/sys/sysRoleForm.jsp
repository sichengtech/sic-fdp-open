...
<html lang="zh-CN">
<head>
<title>${empty role.id?'添加':'编辑'}角色</title>
...
<body>
	...
	<section class="panel">
		<header class="panel-heading custom-tab tab-right ">
			<h4 class="title">${empty role.id?'添加':'编辑'}角色</h4>
			<%@ include file="../include/functionBtn.jsp"%>
			<ul class="nav nav-tabs pull-right">
				<li class=""><a href="${ctxa}/sys/role.do"> <i class="fa fa-home"></i> 角色列表</a></li>
				<shiro:hasPermission name="sys:role:edit">
				<li class="active"><a href="javascript:;"> <i class="fa fa-user"></i> ${empty role.id?'添加':'编辑'}角色</a></li>
				</shiro:hasPermission>
			</ul>
		</header>
		<div class="panel-body">
			...
			<div class="alert alert-info alert-block fade in ${cookie.fdp_operationTips.value=='0'?'point_hidden':''}" id="point">
				<h5>操作提示</h5>
				<ul>
					<li>添加和编辑角色，归属机构可以任意的设置，"角色结构"、"角色名称"、"英文名称"是必填项。</li>
				</ul>
			</div>
			<!-- 提示 end -->
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								<sys:treeselect id="office" name="office.id" value="${role.office.id}" labelName="office.name" labelValue="${role.office.name}"
									title="机构" url="/sys/office/treeData.do" extId="${role.id}" cssClass="required"/>
							</div>
							<div class="col-sm-5">
