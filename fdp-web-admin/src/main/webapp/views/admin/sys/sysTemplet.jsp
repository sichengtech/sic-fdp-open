...
<body>
		...
		<div class="panel-body">
			<div class="col-sm-3" style="height:600px; overflow:auto;">
				<div class="alert alert-default alert-block fade in" style="margin-left: -25px">
					<h5>模板文件列表 
					<button id="treeRefresh" type="button" class="tooltips btn btn-success btn-xs" data-placement="bottom" title="刷新">
			...
			<div class="col-sm-9">
				...
				<div class="alert alert-info alert-block fade in ${cookie.fdp_operationTips.value=='0'?'point_hidden':''}" id="point">
					<h5>操作提示</h5>
					<ul>
						<li><strong>F11</strong> 进入全屏，<strong>Esc</strong> 退出全屏.</li>
						<li><strong>ctrl+space</strong> 代码提示.</li>
						<li>单击Html标签寻找闭合标签(高亮)，<strong>ctrl+J</strong> 跳到配对的闭合标签.</li>
					</ul>
				</div>
				<!-- 提示 end --> 
				<!-- 多行文本框开始 -->
				<div class="form-actions">
					<shiro:hasPermission name="cms:template:edit">
						<button id="create" type="button" class="btn btn-info">
							<i class="glyphicon glyphicon-plus"></i> 创建模板 
						</button>
					</shiro:hasPermission>
				</div>
				<div style="margin-bottom: 10px;margin-top: 10px;">
					当前目录:<span id="path">${name}</span>
					<span id="templateName" style="margin-left:100px; display:none"">
					文件名：<input id="fileName" name="fileName" value="" placeholder="请输入文件名.扩展名"/>
					</span>
				</div>
	...
	<!-- panel end -->
<script type="text/template" id="tplTable_tpl" info="表格模板">
<tr>
	<td>{{d.fileName}}</td>
	<td>{{d.size}}KB</td>
	<td>{{d.lastModifiedDate}}</td>
	<td fullName="{{d.fullName}}" fileName="{{d.fileName}}" dir="{{d.dir}}" isDirectory="{{d.isDirectory}}" >
		<a class="btn btn-info btn-sm editTempateFile" style="margin-right: 5px;"><i class="fa fa-edit"></i> 编辑</a>
		<a class="btn btn-danger btn-sm deleteSure"><i class="fa fa-trash-o"></i> 删除</a>
	</td>
</tr>
</script>	
</body>
