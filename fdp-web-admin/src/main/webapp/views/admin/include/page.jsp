	...
	<div class="page_nav">
		<ul class="pagination pagination-sm pull-left">
			...
			<%} else {// 如果不是首页,"上一页"可点击   pageURL+prev%>
				<li><a href='<%=(page_inner.getPageURL()+page_inner.getPrev())%>'>&laquo;上一页</a></li>
			<%}
			...
			<%} else {// 如果不是尾页,"下一页"可点击%>
				<li><a href='<%=(page_inner.getPageURL()+page_inner.getNext()) %>'>下一页&raquo;</a></li>
			<%}%>
		</ul>
		<span class="pull-left ml15">
			共${page.count}条 ${page.totalPage}页 每页 
			...
			到 <input type="text" id="iptPageTxt" class="form-control" style="width:32px;height:28px;" href="${page.pageURL}" value="${page.pageNo}"
			...
			onclick="(function(obj){var input=document.getElementById('iptPageTxt');if(input.value==''){return;}window.location.href=input.getAttribute('href')+input.value;})(this);">跳转</button>
		</span>
	</div>
</div>
<!-- 分页导航end -->