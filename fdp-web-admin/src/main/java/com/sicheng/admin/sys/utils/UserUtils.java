...
public class UserUtils {
	...
	public static List<Menu> getMenuList() {
		...
		if (user.isAdmin()) {
			...
			menuList = menuDao.selectAll(new Wrapper(new Menu()).orderBy("a.sort"));
		} else {
			//查出用户的角色id
			List<SysUserRole> sysUserRoles = sysUserRoleDao.selectByWhere(null, new Wrapper(new SysUserRole()).and("user_id=",user.getId()));
			if(!sysUserRoles.isEmpty()){
				...
				if(!roles.isEmpty()){
					...
					if(!rIds.isEmpty()){
						...
						if(!sysRoleMenus.isEmpty()){
							...
							//查出菜单
							menuList = menuDao.selectByWhere(null, new Wrapper().and("id in",menuIds).and("del_flag=",BaseEntity.DEL_FLAG_NORMAL).orderBy("sort"));
						}
					}
				}
			}
