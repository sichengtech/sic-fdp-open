...
import com.sicheng.admin.act.dao.ActDao;
import com.sicheng.admin.act.entity.Act;
import com.sicheng.admin.act.utils.ActUtils;
import com.sicheng.admin.act.utils.ProcessDefCache;
...
//							User user = UserUtils.getByLoginName(il.get(0).getStartUserId());
							...
							userQuery.setLoginName(il.get(0).getStartUserId());
							User user = sysUserService.selectOne(new Wrapper(userQuery));
							if (user != null){
...
//					User user = UserUtils.getByLoginName(histIns.getAssignee());
					User userQuery=new User();
					userQuery.setLoginName(histIns.getAssignee());
					User user = sysUserService.selectOne(new Wrapper(userQuery));
					if (user != null){
						e.setAssignee(histIns.getAssignee());
						e.setAssigneeName(user.getName());
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, Long businessId) {
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, Long businessId, String title) {
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, Long businessId, String title, Map<String, Object> vars) {
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId())
