...
<div class="input-group m-bot15">
	<input id="${id}Id" name="${name}" class="form-control ${cssClass}" type="hidden" value="${value}" parentIds="">
    <input id="${id}Name" name="${labelName}" ${allowInput?'':'readonly="readonly"'}  type="text" value="${labelValue}" data-msg-required="${dataMsgRequired}" 
    class="form-control ${cssClass}" style="${cssStyle}"/>
    <span id="${id}Button" class="input-group-addon ${disabled} ${hideBtn ? 'hide' : ''}"><i class="fa fa-search"></i></span>
</div>
</div>
<script type="text/javascript">
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctxa}/tag/treeselect.do?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok","关闭":true}, submit:function(v, h, f){
				if (v=="ok"){ 
					var tree = h.find("iframe")[0].contentWindow.tree;//h.find("iframe").contents();
					var ids = [], names = [], nodes = [],parentIds = [];
					if ("${checked}" == "true"){
					...
					for(var i=0; i<nodes.length; i++) {
						...
						parentIds.push(nodes[i].parentIds);
						<c:if test="${!checked}">
						break; // 如果为非复选框选择，则返回第一个选择  
						</c:if>
					}
					$("#${id}Id").val(ids.join(",").replace(/u_/ig,""));
					$("#${id}Id").attr("parentIds",parentIds.join("#").replace(/u_/ig,""));
